...
function setupDraggableCartWidget() {
    ...
    function onStart(e) {
        ...
        currentX = widget.offsetLeft;
        currentY = widget.offsetTop;
    ...
    function onMove(e) {
        ...
        // Limita movimento dentro da viewport
        ...
        // Detecta proximidade do header (desaparece)
        const header = document.querySelector('header');
        if (header) {
            const headerRect = header.getBoundingClientRect();
            const widgetRect = widget.getBoundingClientRect();

            // Se o widget chegar perto do header (distância < 80px), oculta
            if (widgetRect.bottom - headerRect.bottom < 80 && widgetRect.bottom > headerRect.top) {
    ...
    function updateWidgetBadge() {
        const badge = el('draggableCartBadge');
        if (!badge) return;

        let count = 0;
        Object.keys(cartState.cart).forEach(k => {
            count += cartState.cart[k];
        });
        badge.textContent = count;

        // Se houver itens no carrinho, remove o estado "hidden"
        if (count > 0 && widget.classList.contains('hidden')) {
            widget.classList.remove('hidden');
        }
    }
    ...
    // Clique no widget abre o carrinho
    widget.addEventListener('click', (e) => {
        if (isDragging) return; // não abre se estava arrastando
        const cartDrawer = el('cartDrawer');
        if (cartDrawer) {
