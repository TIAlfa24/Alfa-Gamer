...
(function setupSmartSearch() {
    ...
    const CATEGORIES_MAP = [
        { value: "pc", keywords: ["pc", "computador", "notebook", "desktop", "workstation"] },
        { value: "periféricos", keywords: ["mouse", "teclado", "monitor", "headset", "webcam", "mousepad"] }
    ];
    ...
    function detectCategory(term) {
        term = term.toLowerCase();
        return CATEGORIES_MAP.find(c => c.keywords.some(k => term.includes(k)));
    }
    ...
    function updateSuggestions(input, value) {
        ...
        if (cat) {
            const item = document.createElement("div");
            item.className = "suggestion-item";
            item.innerHTML = `<span>📁 Categoria: ${cat.value}</span>`;
            item.onclick = () => {
                document.querySelector(`input[name="cat"][value="${cat.value}"]`).checked = true;
                applyFilters();
                box.style.display = "none";
            };
            box.appendChild(item);
